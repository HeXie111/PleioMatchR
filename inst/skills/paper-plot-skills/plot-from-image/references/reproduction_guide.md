...
> v2：本指南是 plot-from-image 的 from-scratch 分支。先做分层解析
> （`layer_inventory_schema.json` → `layers.json`），再按以下清单逐项核对，
> 需要反估数据/LaTeX 标签时跳到文末 v2 清单。
...
## Step 5b: Layer Inventory（v2 必做）

对每个视觉元素（坐标轴、误差棒、标注、箭头、图例、inset、填充区域、文本）
记录 `{type, axes_id, bbox_px, style}` 到 `layers.json`。判断线索：

| 层 | 像素线索 |
|----|----------|
| error_bar | 数据点上的须 + 横帽，颜色同系列 |
| filled_region | 两条线之间的半透明色带（alpha<0.4） |
| annotation_box | 白底/浅色底圆角框，内嵌文本 |
| arrow | 带尖头线段，连接两元素 |
| inset | 独立小坐标轴框，常有虚线框指向主图 |
| formula_label | 斜体变量、希腊字母、上下标、分数 |

完成的 `layers.json` 是复现脚本的验收单：脚本必须覆盖每一层。
...
1. 标出 x/y 轴数据极值对应的像素位置（每轴至少 2 点）。
2. 标出每个数据点的像素位置 → `pixel_to_data.py` → CSV。
3. log 轴必须开 `--xlog/--ylog`；断轴/inset 各自独立校准。
4. 提醒用户：反估数据是近似值，正式引用以原文数值为准。

## v2 LaTeX 标签清单

1. OCR/视觉读取轴标签、图例、注释文本。
2. 逐条跑 `latexify_labels.py`（规则库 `latex_rules.json`）。
3. 人工核对上下标/希腊字母；多字符上标记得加 `{}`。
