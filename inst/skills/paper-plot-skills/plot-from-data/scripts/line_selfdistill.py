...
_OUT = Path(__file__).resolve().parents[2] / 'repro'
...
plt.rcParams.update({
    'text.usetex': False,  # enable only when a TeX distro is installed
    'font.family': 'serif',
...
fig2.savefig(_OUT / 'line_selfdistill_train_repro.png',
             dpi=300, facecolor='white')
plt.close(fig2)
print('saved: line_selfdistill_train_repro.png')
...
fig3.savefig(_OUT / 'line_selfdistill_scale_repro.png',
             dpi=300, facecolor='white')
plt.close(fig3)
print('saved: line_selfdistill_scale_repro.png')
